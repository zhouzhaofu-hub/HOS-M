...
import { Smartphone, ShieldCheck, ArrowRight, Bot } from 'lucide-react';
    ...
    <div className="flex flex-col h-full bg-white px-8 pt-16 relative">
      <div className="flex-1 flex flex-col items-center">
        <motion.div
          ...
          className="w-24 h-24 bg-brand-blue rounded-2xl flex items-center justify-center mb-6 shadow-none"
        >
          <Bot className="text-white w-12 h-12" />
        </motion.div>
        
        <h1 className="text-3xl font-black text-text-main tracking-tight">智护OS-M</h1>
        <p className="text-text-muted mt-2 font-medium">Smart Care Terminal v2.4.0</p>
        ...
        <div className="w-full space-y-5 mt-12">
          <div className="relative group">
            ...
            <input
              className="w-full bg-white border border-border-base focus:border-brand-blue rounded-xl py-4 pl-12 pr-4 outline-none text-text-main font-bold transition-all"
              placeholder="管理员账号"
          ...
          <div className="relative group">
            ...
            <input
              ...
              type="password"
            />
            <button className="absolute right-4 top-1/2 -translate-y-1/2 text-brand-blue font-bold text-sm">获取</button>
          </div>

          <button
            onClick={handleLogin}
            disabled={loading}
            className="w-full bg-brand-blue text-white py-4 rounded-xl font-black text-lg active:scale-[0.98] transition-all flex items-center justify-center gap-2 group"
          >
            {loading ? (
        ...
        <div className="w-full mt-12">
          <div className="flex items-center gap-4 mb-6">
            ...
            <span className="text-xs font-bold text-slate-300 uppercase tracking-widest">快速登录</span>
            <div className="h-px flex-1 bg-slate-100"></div>
          </div>
          <div className="flex justify-center gap-6">
            <button className="w-14 h-14 rounded-2xl border border-slate-100 flex items-center justify-center hover:bg-slate-50 transition-colors">
